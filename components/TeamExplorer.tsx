...
import type { ConstructorHistoryResponse } from "@/app/api/teams/history/route";
...
type SortField = "name" | "debutSeason" | "lastSeason" | "seasonsRaced" | "championships" | "wins" | "points";
...
export default function TeamExplorer({ currentStandings }: { currentStandings: ConstructorStanding[] }) {
  const [view, setView] = useState<View>("current");
  const [history, setHistory] = useState<ConstructorHistoryResponse | null>(null);
  ...
  async function loadHistory() {
    ...
    try {
      ...
      const data: ConstructorHistoryResponse = await res.json();
      setHistory(data);
      setSelectedYear((prev) => prev || data.seasons[data.seasons.length - 1] || "");
    } catch (err) {
      console.error("Failed to load team history:", err);
      setHistoryError("Couldn't load full constructor history — the data source may be temporarily unavailable.");
    } finally {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            {view === "current" && "Current season, ranked by championship points"}
            {view === "byYear" && "Final standings for any season since 1950"}
            {view === "allTime" && "Every constructor in F1 history"}
          </p>
        </div>
      ...
      {view === "current" &&
        (currentStandings.length > 0 ? (
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Standings not available for the current season.</p>
        ))}

      {view !== "current" && historyLoading && (
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Loading full constructor history — this pulls every F1 season since 1950, so it can take a little while
          the first time. It&apos;s cached after that.
        </p>
      )}
      ...
      {view === "byYear" && history && (
        <div className="flex flex-col gap-4">
          ...
          {history.standingsBySeason[selectedYear] ? (
            <StandingsTable type="constructor" standings={history.standingsBySeason[selectedYear]} />
          ) : (
        ...
        <div className="flex flex-col gap-4">
          <div className="overflow-x-auto rounded-lg border border-black/10 dark:border-white/10">
            <table className="w-full min-w-[520px] text-left text-sm">
              <thead className="bg-black/[.03] dark:bg-white/[.05] text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                <tr>
                  <SortableHeader field="name" label="Constructor" sortField={sortField} sortDir={sortDir} onSort={toggleSort} />
                  <SortableHeader
                    field="debutSeason"
                    label="Debut"
                    sortField={sortField}
              ...
              <tbody className="divide-y divide-black/5 dark:divide-white/10">
                {pageItems.map((team) => (
                  <TeamRow key={team.constructorId} team={team} />
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div className="flex items-center justify-between text-sm">
            <p className="text-zinc-600 dark:text-zinc-400">
              {sortedDirectory.length} constructors · page {currentPage} of {totalPages}
            </p>
...
function TeamRow({ team }: { team: ConstructorDirectoryEntry }) {
  return (
    <tr className="hover:bg-black/[.02] dark:hover:bg-white/[.03]">
      <td className="px-4 py-2">
        <Link href={`/teams/${team.constructorId}`} className="font-medium hover:underline">
          {team.name}
        </Link>
        <span className="ml-2 text-xs text-zinc-500 dark:text-zinc-400">{team.nationality}</span>
      </td>
      <td className="px-4 py-2">{team.debutSeason}</td>
      <td className="px-4 py-2 text-right">{team.seasonsRaced}</td>
      <td className="px-4 py-2">
        {team.isCurrent ? (
          ...
          <span className="text-zinc-600 dark:text-zinc-400">Retired {team.lastSeason}</span>
        )}
      </td>
      <td className="px-4 py-2 text-right">{team.championships || "—"}</td>
      <td className="px-4 py-2 text-right">{team.wins}</td>
      <td className="px-4 py-2 text-right font-medium">{team.points}</td>
    </tr>
  );
}
